...
function validateProductionEnv(env) {
  ...
  if (!/^[a-z][a-z0-9_-]{2,31}$/.test(env.MONITOR_USER || '')) errors.push('MONITOR_USER must be a valid dedicated broker username');
  if (!env.MONITOR_PASS || env.MONITOR_PASS.length < 12) errors.push('MONITOR_PASS must be at least 12 characters');
  if (env.MONITOR_USER === (env.DYNSEC_ADMIN_USER || 'dynsec-admin')) errors.push('MONITOR_USER must not be the dynsec administrator');
