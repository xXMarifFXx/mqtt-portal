...
async function createStudent(user, pass) {
  ...
  const ns = nsFor(user);
  await run(args.createClient(c, user, pass)); // throws if the username is taken
  // Per-user role scoped to devices/<user>/# (literal — no %u).
  await run(args.createRole(c, role)).catch(() => {});
  await run(args.addRoleACL(c, role, 'subscribePattern', ns)).catch(() => {});
  await run(args.addRoleACL(c, role, 'publishClientSend', ns)).catch(() => {});
  await run(args.addRoleACL(c, role, 'publishClientReceive', ns)).catch(() => {});
  await run(args.addClientRole(c, user, role));
}
...
module.exports = {
  cfg, connFlags, args, parseClients, roleFor, nsFor,
  createStudent, resetPassword, deleteStudent, disableStudent, enableStudent, listStudents,
};
