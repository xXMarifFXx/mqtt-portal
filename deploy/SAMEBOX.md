...
Set the separate read-only monitor credential from step 2 as `MONITOR_PASS` and leave
`MONITOR_USER=portal-monitor`; never use the dynsec administrator for monitoring.
Then lock it down:
```bash
